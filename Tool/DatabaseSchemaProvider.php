 ...
 *
 */
class Pincrowd_Tool_DatabaseSchemaProvider extends Zend_Tool_Project_Provider_Abstract
{
    /**
    ...
    public function updateTo($version, $env='development', $dir='./scripts/migrations')
    {
        ...
        try {
            ...
            $manager = new Pincrowd_Db_Schema_Manager($dir, $db, $this->getTablePrefix());

            $result = $manager->updateTo($version);

            switch ($result) {
                case Pincrowd_Db_Schema_Manager::RESULT_AT_CURRENT_VERSION:
                    if (!$version) {
                ...
                case Pincrowd_Db_Schema_Manager::RESULT_NO_MIGRATIONS_FOUND :
    ...
    protected function _init($env)
    {
        ...
        $autoloader->registerNamespace('Pincrowd_');
    }
    ...
    protected function _getDbAdapter()
    {
        if ((null === $this->_db)) {
            ...
            if(!$this->_db instanceof Zend_Db_Adapter_Abstract) {
                throw new Pincrowd_Db_Schema_Exception('Database was not initialized');
            }
        }
        return $this->_db;
    }
    ...
    public function createMigration($migrationName){
        ...
        $data .= $cg->setName($migrationName)
            ->setExtendedClass('Pincrowd_Db_Schema_AbstractChange')
            ->setMethods($methods)
